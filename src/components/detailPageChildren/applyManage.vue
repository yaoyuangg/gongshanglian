<template>
    <div class="container searchBody">
          ...
          <div class="row">
            <table class="table table-bordered" v-show="stateControl.tableShow">
              ...
              <tr v-for="(item, index) in tableInfos" v-bind:key="index">
                ...
                <td>{{item.state === null ? '待审批' : (item.state === true ? '审批通过' : '审批未通过')}}</td>
                <td style="text-align: center">
                  <button class="btn btn-primary" @click="toDetail(item.industryName, item.id)">详情</button>
                  <button class="btn btn-danger" v-show="(item.id && (roleId === '3'))" @click="toDelete(item.id)">删除</button>
                  <button class="btn btn-danger" v-show="item.id && item.state === null && (roleId === '3')" @click="toAllow(item.id)">审批</button>
            ...
            <div class="modal-body">
              <form class="container">
                <div class="row">
                  ...
                  <div class="col-md-4">
                    <input class="form-control" type="text" v-model="productInfo.industryName">
                  </div>
                </div>
      ...
      <!--产品信息详情-->
      <div class="modal fade" id="detailApply" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
            ...
            <div class="modal-body">
              <form class="container">
                <div class="row">
                  ...
                  <div class="col-md-2"><span class="red">*</span>手机号码:</div>
                  <div class="col-md-4">
                    {{productInfo.phone}}
                  </div>
                </div>
      ...
      <!--删除-->
      <div class="modal fade" id="deleteApply" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
      ...
      <!--审批-->
      <div class="modal fade" id="allowApply" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal" @click="confirmAllow('false')">不通过
              </button>
              <button type="button" class="btn btn-primary" @click="confirmAllow('true')">
                通过
              </button>
            </div>
          </div><!-- /.modal-content -->
...
export default {
  ...
  methods: {
    ...
    */
      ...
      this.request(
        ...
        function (res) {
          ...
          if (res.data.list.length === 0) {
            ...
            // that.$emit('updateIndustry', res.data.businesses)
    ...
    toAdd () {
      this.fail = false
      $('#addApply').modal({backdrop: 'static', keyboard: false})
      this.productInfo = {
        industryName: ''
      }
    },
    ...
    toDelete (id) {
      this.id = id
      $('#deleteApply').modal('show')
    },
    toAllow (id) {
      this.id = id
      $('#allowApply').modal('show')
    },
    /**
     * 函数描述
     *  确认审批
...
.btn-Apply>div{
  ...
  /* ==========================================================================
   模态框
 ============================================================================ */
  #addApply .modal-dialog,
  #detailApply .modal-dialog{
    width: 60%;
  }
  .file{
    text-align: left
